// Second pass of the physarum simulation: Diffuse and decay the simulation space
...
[numthreads(30, 30, 1)] void main(uint3 i : SV_DispatchThreadID)
{
    int texWidth;
    int texHeight;
    ...
    // Wait: This actually would require double buffering!

    float3 sumNeighbours =
        (WriteOutput[i.xy + int2(0, 1)].rgb    //
         + WriteOutput[i.xy + int2(0, -1)].rgb //
         + WriteOutput[i.xy + int2(-1, 0)].rgb //
         + WriteOutput[i.xy + int2(+1, 0)].rgb //
         + WriteOutput[i.xy].rgb) /
        5; //
    ...
    WriteOutput[i.xy] = diffused;
    // + float4(fx.rgb, 0) * fx.a;
